...
template < class Key, class T, class Compare = std::less< Key >, class Allocator = std::allocator< pair< const Key, T > > > class map {
      ...
      public:
        ...
        map() : _comp(key_compare()), _allocator(allocator_type()), _size(0), _end(_tree_alloc.allocate(sizeof(tree))), _begin(_end), _root(NULL) { _tree_alloc.construct(_end, tree()); }

        explicit map(const key_compare &comp, const allocator_type &alloc = allocator_type()) : _comp(comp), _allocator(alloc), _size(0), _end(_tree_alloc.allocate(sizeof(tree))), _begin(_end), _root(NULL) { _tree_alloc.construct(_end, tree()); }

        template < class Iter > map(Iter first, Iter last, const key_compare &comp = key_compare(), const allocator_type &alloc = allocator_type(), typename enable_if< !is_integral< Iter >::value, Iter >::type * = 0) : _comp(comp), _allocator(alloc), _size(0), _end(_tree_alloc.allocate(sizeof(tree))), _begin(_end), _root(NULL) {
                _tree_alloc.construct(_end, tree());
                insert(first, last);
        }

        map(const map &other) : _comp(other.key_comp()), _allocator(other.get_allocator()), _size(0), _end(_tree_alloc.allocate(sizeof(tree))), _begin(_end), _root(NULL) {
                _tree_alloc.construct(_end, tree());
                *this = other;
        }

        ~map() { destroy_content(_root); }
        ...
        size_type max_size(void) const { return (_tree_alloc.max_size() + _allocator.max_size()) / 2; }
        ...
        template < class Iter > void insert(Iter first, Iter last, typename enable_if< !is_integral< Iter >::value, Iter >::type * = 0) {
      ...
      private:
        ...
        class tree {
              public:
                // CONSTRUCTEURS + DESTRUCTEUR
                tree(const allocator_type &alloc = allocator_type()) : _allocator(alloc), _pair(_allocator.allocate(sizeof(value_type))), _parent(NULL), _child1(NULL), _child2(NULL), _level(0) { _allocator.construct(_pair, value_type()); }

                tree(const_reference new_pair, tree *parent, tree *child1, tree *child2, const size_type level, const allocator_type &alloc = allocator_type()) : _allocator(alloc), _pair(_allocator.allocate(sizeof(value_type))), _parent(parent), _child1(child1), _child2(child2), _level(level) { _allocator.construct(_pair, new_pair); }

                tree(const tree &other) : _allocator(other._allocator), _pair(_allocator.allocate(sizeof(value_type))), _parent(other._parent), _child1(other._child1), _child2(other._child2), _level(other._level) { _allocator.construct(_pair, *(other._pair)); }
...
template < class Key, class T, class Compare, class Alloc > void swap(map< Key, T, Compare, Alloc > &lhs, map< Key, T, Compare, Alloc > &rhs) { lhs.swap(rhs); }
} // namespace ft
#endif
