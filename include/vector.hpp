...
template < class T, class Allocator = std::allocator< T > > class vector {
      public:
        ...
        vector(void) : _allocator(allocator_type()), _capacity(0), _size(0), _container(NULL) {}

        vector(const vector &other) : _allocator(other._allocator), _capacity(0), _size(0), _container(NULL) { *this = other; }

        explicit vector(const allocator_type &alloc) : _allocator(alloc), _capacity(0), _size(0), _container(NULL) {}

        explicit vector(size_type count, const value_type &value = value_type(), const allocator_type &alloc = allocator_type()) : _allocator(alloc), _capacity(0), _size(0), _container(NULL) { assign(count, value); }

        template < class Iter > vector(Iter first, Iter last, const allocator_type &alloc = allocator_type(), typename enable_if< !is_integral< Iter >::value, Iter >::type * = 0) : _allocator(alloc), _capacity(0), _size(0), _container(NULL) { assign(first, last); }

        ~vector() { destroy_content(); }
        ...
        friend bool operator==(const vector &lhs, const vector &rhs) { return lhs.size() == rhs.size() && ft::equal(lhs.begin(), lhs.end(), rhs.begin()); }
        ...
        pointer data(void) { return _container; }

        const_pointer data(void) const { return _container; }

        // ITERATEURS
        iterator begin(void) { return iterator(&(_container[0])); }

        const_iterator begin(void) const { return const_iterator(&(_container[0])); }
        ...
        template < class Iter > void assign(Iter first, Iter last, typename enable_if< !is_integral< Iter >::value, Iter >::type * = 0) {
                vector temp;
        ...
        template < class Iter > void insert(iterator pos, Iter first, Iter last, typename enable_if< !is_integral< Iter >::value, Iter >::type * = 0) {
                vector temp;
                for (; first != last; first++)
                        temp.push_back(*first);
...
template < class value_type, class allocator_type > void swap(vector< value_type, allocator_type > &lhs, vector< value_type, allocator_type > &rhs) { lhs.swap(rhs); }
} // namespace ft
#endif
